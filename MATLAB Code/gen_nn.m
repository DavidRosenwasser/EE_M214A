function [EER,net] = TrainNet(trainFeatures,trainLabels, testFeatures, testLabels)
%% Description
...
%
% Outputs:    
%   - EER: Equal error rate of the classifer (used for scoring metrics)  
...
%% Input test data into trained net
testOutput = net(testFeatures);
[EER,~] = compute_eer(testOutput, testLabels);
disp(['Computed EER is ' num2str(EER)]);
